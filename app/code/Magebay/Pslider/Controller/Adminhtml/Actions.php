        ...
        try {
			...
			//if (!empty($_FILES['image']['name'])) {
				try {
						...
						$uploader->addValidateCallback('news_image', $imageAdapter, 'validateUploadFile');
						...
						$result = $uploader->save($mediaDirectory->getAbsolutePath(\MST\News\Model\News::BASE_MEDIA_PATH));
						$params['image'] = \MST\News\Model\News::BASE_MEDIA_PATH . $result['file'];
					} catch (\Exception $e) {
						...
						if (isset($params['image']) && isset($params['image']['value'])) {
							if (isset($params['image']['delete'])) {
								$params['image'] = null;
								$params['delete_image'] = true;
							} else if (isset($params['image']['value'])) {
